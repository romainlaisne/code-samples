...
//require ($serverDocumentRoot . '/my3form_europe/optimizer/fpdf17/fpdf.php');
require ($serverDocumentRoot . '/my3form_europe/optimizer/tfpdf/tfpdf.php');
//require ($serverDocumentRoot . '/my3form_europe/optimizer/tcpdf/tcpdf.php');//couldn't get this working.  Not compatible with current code
//require_once ($serverDocumentRoot . '/my3form_europe/optimizer/fpdi/fpdi.php');
...
class FPDF extends tFPDF {
	...
	function Header() {
		...
		//$title="Reports - Project: ".$_SESSION['quotedescriptioncutreport'];//$_SESSION['projectquotefulldesc'];
...
//$this->SetFont('DejaVu','I',8);
		...
		$alias = "{nb}";
//		$alias = iconv("ISO-8859-1","UTF-8//IGNORE","{nb}");
		$this -> Cell(0, 10, 'Page ' . $this -> PageNo()/*.'/'.$alias*/, 0, 0, 'C');//FIXME alias doesn't work with utf8
	...
	function ChapterTitle($num, $label) {
		...
		$this -> SetFillColor(138,181,255);//(200, 220, 255);
	...
	function productTable($summary,$column3,$column2,$quoteid,$text){
		...
		$this->SetFillColor(112,112,112);//Make gray per romain
		...
		{
			$this->Cell($w[0],5,$item[0],'LR',0,'L',$fill); // line 73
			$this->Cell($w[1],5,$item[1],'LR',0,'L',$fill); // line 74
			//$this->Cell($w[2],5,$item[2],'LR',0,'L',$fill); // line 75
...
//		//build 2 column section
//		$item = array(); 
//		foreach($column2 as $item)
//		{
//			$this->SetFont('','B');
//			$this->Cell(21,5,$item[0],'LR',0,'L',$fill); // line 73
//			$this->SetFont('');
//			$this->Cell(21,5,$item[1],'LR',0,'L',$fill); // line 74
//			
// 	
//			$this->Ln();
//			$fill=!$fill;
...
//	$nbtest = $this->NbLines(21, $value[0]);//this doesn't work

	//jjh 16042012: THis is a hack since the utf-8 truetype font dejavu breaks nbLines...  
	...
	$this->Row(array($value[0], $value[1]), $count);//.":".$count  //." $count1:$count2"
}
...
//$this->SetY($y);
			...
			$this->SetFont('');
			$this->MultiCell($width, $height, "".$item[1]."", 'LR', 'L', $fill);//LR
			
			$x=$this->GetX()-$width;
			$y=$this->GetY()-$height;
			
			//adjust for extra lines so there are no breaks in lines
			$this->GetY()-$height;
			$this->SetY($y);
			$this->MultiCell(1, $height, '', 'L', 'L', $fill);//LR// fix for 3 wrapping rows?
			$this->SetX($x);
...
//		}
		...
		foreach($data as $item){
			...
			//$this->Cell($w[0],5,$item[0],'LR',1,'L',$fill); // line 73
	
			//build left line
//			$this->MultiCell($width,$height,'','L','L','');//
			$this->SetY($y); //set pointer back to previous values
			$this->SetX($x);
...
function Row($data, $in_nb=0, $fill=array())
{
    ...
    $usefill = 0;
    if(!empty($fill)){
    	$this -> SetFillColor($fill[0],$fill[1],$fill[2]);//fill color
    	$usefill = 1;
    }
    ...
    for($i=0;$i<count($data);$i++)
    {
        ...
        $this->Rect($x, $y, $w, $h);
        //Print the text
        $this->MultiCell($w, 4, $data[$i], 0, $a, $usefill);//$this->MultiCell($width, $height, "".$item[1]."", 'LR', 'L', $fill);
...
$pdf = new FPDF();
//$title = utf8_encode('Cut report - Quote' . $_SESSION[quote]);
//
//$pdf -> SetTitle($title);
$pdf -> Header();
$pdf -> SetAuthor('3form');
...
//$alias = iconv("ISO-8859-1","UTF-8//IGNORE","{nb}");//.iconv("UTF-8", "ISO-8859-1",$fabdesr['text']['rep_panels'])
$alias = "{nb}";
$pdf->AliasNbPages();
//$pdf->SetFont('DejaVu','',7);
//$nbtest = $pdf->NbLines(21, "this is a lot of text this is a lot of text this is a lot of text");//
//$nbtest = $pdf->NbLines(21, $r_reptext['rep_colouronfront']);//
...
if ($_SESSION) {
	...
	// $pdf -> PrintChapter(1, 'A RUNAWAY REEF', 'http://www.id.uzh.ch/cl/zinfo/fpdf/tutorial/20k_c1.txt');
	// $pdf -> PrintChapter(2, 'THE PROS AND CONS', 'http://www.id.uzh.ch/cl/zinfo/fpdf/tutorial/20k_c2.txt');

	// $img1=$serverDocumentRoot.'/my3form_europe/optimizer/images/195/set0sheet0.png';
	// $pdf -> Image($img1, 10, 6, 30);
	...
	if (count($_SESSION['img_arr']) > 0 && $report['cut'] == 1) {
		...
		for ($i = 0; $i < count($_SESSION['img_arr']); $i++) {
			...
			//echo "<br><b>$r_reptext[rep_patterndir]: </b>";
	...
	if (count($_SESSION['img_glass_arr']) > 0 && $report['cut'] == 1) {
		...
		// $set1=$pdf -> Image($img1).$pdf -> Image($img1).$pdf -> Image($img1).$pdf -> Image($img1);
		// $resp -> addAssign("processingPDFExport", "innerHTML", $set1);
		// $pdf -> PrintChapter(2, 'CHROMA', $set1);

		// $pdf -> PrintChapter(4, 'IMG', $pdf -> Image($img1));
		// $pdf -> PrintChapter(4, 'IMG', $pdf -> Image($img1));
	}
	...
	if($report['highres'] == 1){
		...
		$pdf -> SetCol(0);//set margins back to default
		//$pdf->SetAutoPageBreak (false, -45);//this is important to get the 4 box layout to go to the bottom of the page near the page count
				...
				while($r_hr = mysql_fetch_assoc($q_hr)){
					...
					if($firstsubhr == true){
						if($processgroup == false){			
							$pdf -> PrintChapter(1, "$r_reptext[rep_panel] $r_quoteprod[id] $hashighres[prod_name]", '');// Header 
							$firstsubhr = false;
						}else{
							$pdf -> PrintChapter(1, "$r_reptext[rep_panel] $r_quoteprod[id] $hashighres[prod_name] - $r_reptext[rep_all] $r_quoteprod[qty] $r_reptext[rep_panels] -", '');// Header 
							$firstsubhr = false;
						}
					...
					//** build images
					//$pdf->Image("https://dev.3form.eu/images/highres/individuals/$r_hr[fab_detail_2].jpg",$x,$y,30,'','JPG');
					...
					$pdf->SetX($x);
					$pdf->MultiCell(32,8," $r_hr[fab_detail_2]",0,'L',0);//LR $r_quoteprod[length]
					$pdf->SetY($y);
	...
	if($report['leadtimes'] == 1){
		...
		if($get_fabrication['pricing'] > 750){
			$totalleaddays = ((round($get_fabrication['leadtimes']['total_manmin']) + round($get_fabrication['leadtimes']['cnch']) 
				+ round($get_fabrication['leadtimes']['ovenh']) + round($get_fabrication['leadtimes']['vacth']))/8/60)+10;
		...
		//** build table
		$pdf -> PrintChapter(1, "Lead Times", '');// Header 

		//$pdf -> SetFont('Arial', 'I', 8);
		
		$cw = 47.5;//cw = Cell Width
		...
		$fullwidth = $cw * 4;//the full width of the page for header rows
		$halfwidth = $cw * 2;//the full width of the page for header rows
		$pdf -> SetFillColor(225,222,201);//fill color
		
		//$font = "helvetica";
		$pdf->SetFont($font,'B',$fontsize);
    	$pdf->Cell($fullwidth,$lineheight,"Fabrication lead time",1,0,'L',1);////LR
    	$pdf->Ln();
    	
    	$pdf -> SetFillColor(206,201,166);//fill color
    	$pdf->Cell($halfwidth,$lineheight,"Fab. Lead time indication (w. days), conf. req.",1,0,'L',1);////LR
    	$pdf->Cell($halfwidth,$lineheight,round($totalleaddays)." days",1,0,'L',1);////LR
    	$pdf->Ln();
    	
    	//set font back to no bold
    	//$pdf->SetFont($font,'',$fontsize);

    	$pdf -> SetFillColor(238,236,225);//fill color
		...
		for ($c=0;$c<3;$c++){	
			...
			if($c==0){
				...
				$pdf->Row(array("Machine"),"",$fill);$pdf->SetX($x + 5);
							
				$pdf->SetWidths(array(30, 15));
				//srand(microtime()*1000000);
				
				$pdf->SetFont($font,'',$fontsize);
				$pdf->Row(array("CNC H. (Min)", round($get_fabrication['leadtimes']['cnch'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Oven H. (Min)", round($get_fabrication['leadtimes']['ovenh'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Vac.t. H. (Min)", round($get_fabrication['leadtimes']['vacth'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Line Bend H. (Min)", round($get_fabrication['leadtimes']['linebendh'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Saw H. (Min)", round($get_fabrication['leadtimes']['sawh'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Frees. H. (Min)", round($get_fabrication['leadtimes']['freesh'])),"",$fill);$pdf->SetX($x + 5);
				$pdf->Row(array("Handtools H. (Min)", round($get_fabrication['leadtimes']['handtools'])),"",$fill);$pdf->SetX($x + 5);
				...
				$pdf->Row(array("Human"),"",$fill);$pdf->SetX($x);
							
				$pdf->SetWidths(array(40, 40));
				//srand(microtime()*1000000);
				
				$pdf->SetFont($font,'',$fontsize);
				$pdf->Row(array("CNC F. ManMin", round($get_fabrication['leadtimes']['cncf_manmin'])),"",$fill);$pdf->SetX($x);
				$pdf->Row(array("Compl. F. ManMin", round($get_fabrication['leadtimes']['complf_manmin'])),"",$fill);$pdf->SetX($x);
				$pdf->Row(array("Lite F. ManMin", round($get_fabrication['leadtimes']['litef_manmin'])),"",$fill);$pdf->SetX($x);
	
	
				$pdf->SetFont($font,'B',$fontsize);
				$pdf->Row(array("Total ManMin", round($get_fabrication['leadtimes']['total_manmin'])),"",$fill);$pdf->SetX($x);
	...
	if( $report['fab'] == 1){
		...
		$q_corpaddr = db_query("SELECT u.*, ua.*, u.phone as uphone FROM users u INNER JOIN useraddr ua ON ua.uid = u.uid where u.uid ='76'");//corporate account
		$r_corpaddr = mysql_fetch_assoc($q_corpaddr);
		$logoaddress = "<br>".nl2br($r_corpaddr['addr']).
					"<br>The Netherlands".
					"<br>$r_corpaddr[uphone]".
					"<br>$r_corpaddr[fax]".
					"<br>Web www.3form.eu".$nbtest;
		...
		while ($r_sh = mysql_fetch_assoc($q_sh)) {
			...
			//$panel_id_limit = $r_sh['id']."_".$r_sh['qty'];
			
			//** get all fab
			$q_fab = db_query("SELECT * FROM qt_fab_products WHERE quoteproduct_id ='$r_sh[id]' AND active = '1' AND 
				(fab_category = 'otherfab' OR panel_num <= '$r_sh[qty]') AND fab_category != 'highres' ORDER BY panel_id ASC");//remove otherfab?
			// panel_id <='$panel_id_limit' OR 
			$count_fab = mysql_num_rows($q_fab);//make sure there is at least one fabrication
			
			$processgroup = true;//$uniqueprod = 
			...
			if($count_fab > 0){//there is fabrication, so proceed
				while($r_fab = mysql_fetch_assoc($q_fab)){
					...
					//echo "<pre>".print_r($fabdesr,1)."</pre>";
					if($r_fab['process_group'] == "0"){//process individually
						$processgroup = false;
					}
				}
				
				if(!isset($fabdesr['projectquote']) && !isset($fabdesr['quotename']) && !isset($fabdesr['quote_number'])){ continue; }//move to next because it's empty
		
				$loopcount = ($processgroup == false ? 1 : $r_sh['qty']);//all sheets are process group, set loopcount to qty
				//$firephp->warn ($r_sh['qty'],"loop quantity");
				while($loopcount <= $r_sh['qty']){                   
					$drawingnumtotal++;
					$loopcount++;
					$drawingnumtotal = str_pad($drawingnumtotal, 3, "0", STR_PAD_LEFT);//pad drawing number with leading zeros
				}
			}
		}
		...
		while ($r_sh = mysql_fetch_assoc($q_sh)) {
			...
			if($count_fab > 0){//there is fabrication, so proceed
				...
				while($loopcount <= $r_sh['qty']){
					...
					$panelnum = $allpanels = "";
					if($processgroup == false){
						//$panelnum = "$loopcount of ".$r_sh['qty'];
						$panelnum = "$loopcount";			
						$fabheader = $fabdesr['text']['rep_panel']." $r_sh[id]:$loopcount - ".
							ucwords($fabdesr['prod_name'])." ".$fabdesr['text']['rep_fabrication']."";
						$pdf -> PrintChapter(1, $fabheader, '');// Header  
					}else{
						$panelnum = $fabdesr['text']['rep_all'];//"1-".$r_sh['qty']; //." ".iconv("UTF-8", "ISO-8859-1",$fabdesr['text']['rep_panels'])
						$allpanels = ($r_sh['qty'] > 1 ? "- ".$fabdesr['text']['rep_all']." $r_sh[qty] ".
							$fabdesr['text']['rep_panels']." -" : "");
						$fabheader = $fabdesr['text']['rep_panel']." $r_sh[id] $allpanels ".
							ucwords($fabdesr['prod_name'])." ".$fabdesr['text']['rep_fabrication']."";
						$pdf -> PrintChapter(1, $fabheader, '');// Header 
					}
					...
					if(!empty($holesarr)){
						foreach ($holesarr as $key => $value) {
							...
							$holesstring .= "$value[left],$value[top],$value[size];";//$key=
					...
					$masterimage = "my3form_europe/vsample/10/
						fab:master-l:$sheetlength-w:$sheetwidth-a:".$fabdesr[$loopcount]['align']."-arr:".$fabdesr[$loopcount]['holeid'].
						"-r:$ran".$fabdesr[$loopcount]['b1'].$fabdesr[$loopcount]['b2']."-lang:$r_pq[language_id]-hs:$holesstring.png";
					

//					$pdf->setY(80);
//					$pdf->setX(50);
//					$pdf->multicell(100,$lineheight,$masterimage,'TLR',1,'C',1);//$masterimage print_r($_SESSION['fab_arr'][102],1)
					
//					$displaymasterimage = "";// $pdf->Image("http://$SERVER$masterimage",60,30,90,0,'PNG');
					...
					foreach($parts_arr as $type => $column){
						if($layers_arr[$type] != ""){
							$allproducts = explode(" ",$layers_arr[$type]); //echo "<br />Layers: ".$layers_arr[$type]." and type: $type";
							$pid_names = array();
							foreach($allproducts as $key => $prodid){
								...
								//$pid_names[] = convertPIDField($column,$pidkey);echo "<br />COlumn: $column, PIDKEY:$pidkey";
							}
							${$type} = implode(", ",$pid_names); //echo "<br />THis is new $type: ".${$type};
						}
					}
					...
					$cw = 42;//cw = Cell Width
					$firstcell = 22;//first cell, smaller than the rest
					$cellsplit = $cw / 4;//used for holes
					$fullwidth = $cw * 4 + $firstcell;//the full width of the page for header rows
					$pdf -> SetFillColor(138,181,255);//fill color
					...
					if(!empty($fabdesr[$loopcount]['fabed1']) || !empty($fabdesr[$loopcount]['fabef1']) || !empty($fabdesr[$loopcount]['fabmilling1'])){
						...
						if(!empty($fabdesr[$loopcount]['fabef1'])){
							...
							$pdf->SetFont($font,'B',$fontsize);
		    				$pdf->Cell($fullwidth,$lineheight,$fabdesr['text']['rep_edgefinishes'],1,0,'L',1);////LR
		    				$pdf->Ln();
						...
						if(!empty($fabdesr[$loopcount]['fabed1'])){
							...
							$pdf->SetFont($font,'B',$fontsize);
		    				$pdf->Cell($fullwidth,$lineheight,$fabdesr['text']['rep_edgedetails'],1,0,'L',1);////LR
		    				$pdf->Ln();
						...
						if(!empty($fabdesr[$loopcount]['fabmilling1'])){
							...
							$pdf->SetFont($font,'B',$fontsize);
		    				$pdf->Cell($fullwidth,$lineheight,$fabdesr['text']['rep_milling'],1,0,'L',1);////LR
		    				$pdf->Ln();
...
//					//$pdf -> MultiCell(190, 20, ucwords($fabdesr['prod_name'])."", 0, 1, 'L', false);
//					$pdf->Cell(190,$lineheight,ucwords($fabdesr['prod_name']),'',0,'L','');//TLR
//					$pdf->Ln();
//					
//					$pdf->SetFont('Helvetica','',$fontsize);
//					$pdf->Cell(190,$lineheightsmall,"Front Finish: $r_sh[ff]",'',0,'L','');//LR
//    				$pdf->Ln();
//    				$pdf->Cell(190,$lineheightsmall,"Back Finish: $r_sh[bf]",'',0,'L','');//LR
//    				$pdf->Ln();
//    				$pdf->Cell(190,$lineheightsmall,"Gauge: $r_sh[gauge]",'',0,'L','');//LR
//    				$pdf->Ln();
//    				$pdf->Cell(190,$lineheightsmall,"Quantity: $r_sh[qty]",'',0,'L','');////LR
//    				$pdf->Ln();
//    				$pdf->Cell(190,$lineheight,"",'',0,'L','');//LR
//    				$pdf->Ln();
//    				
//    				$pdf->SetFont('Helvetica','B',$fontsizeh1);
//					$pdf->Cell(190,$lineheight,"Fabrication:",'',0,'L','');//LR
//					$pdf->Ln();
//					
//					$pdf->SetFont('Helvetica','',$fontsize);
//					$fabtext = ($processgroup == true ? $fabdesr[0]['descr'] : $fabdesr[$loopcount]['descr']);
//					$fabtext_arr = explode("<br />", $fabtext);
//					foreach($fabtext_arr as $key => $textvalue){
//						$pdf->MultiCell(190, $lineheightsmall, $textvalue, '', 'L', '');//LR
//					}
					//$pdf->Cell(190,$lineheight,"",'T',0,'L','');
					$pdf->Ln(0);
					
					//Holes
					//if(!empty($fabdesr[$loopcount]['toprow'])){
					//$loopcount=0;
					
					//**#181, some holes are processed globally, analyse fabdesr[0]                
                    /*if(array_key_exists('toprow',$fabdesr[0])){
                        $firephp->warn("### HOLES GLOBAL");
                        define('DIAMETER', chr(248));//"&#216;");//create diameter symbol since &Oslash; doesn't work outside html  //chr(248)
                        
//                      //set title
//                      $pdf->SetFont('Helvetica','B',$fontsizeh1);
//                      $pdf->Cell(190,$lineheight,"Holes:",'',0,'L','');////LR
//                      $pdf->Ln();
                        ...
                        foreach($fabdesr[0]['drow'] as $key => $holedet){
					...
					//if(array_key_exists('toprow',$fabdesr[$loopcount])){
...
//	    				$pdf->Ln();
					...
					if(!empty($fabdesr[$loopcount]['line_descr'])){
						...
						$pdf->SetFont($font,'',$fontsize);
						$pdf->MultiCell($cw * 4, $lineheight, $fabdesr[$loopcount]['line_descr'], 1, 'L', '');//LR
						$pdf->Ln();
					}
					...
					if(!empty($fabdesr[$loopcount]['shape_descr'])){
						...
						$pdf->SetFont($font,'',$fontsize);
						$pdf->MultiCell($cw * 4, $lineheight, $fabdesr[$loopcount]['shape_descr'], 1, 'L', '');//LR
						$pdf->Ln();
					}
					...
					if($fabdesr[$loopcount]['fadeimg'] != ""){
						...
						$fadedesc1 = $fabdesr[$loopcount]['fadedescr1'];//.' Width: '.$sheetwidth." imgwidth".$imgwidth
						$fadedesc2 = $fabdesr[$loopcount]['fadedescr2'];//.$imglength
					}
	...
	$pdf->Cell(60,$lineheightsmall,$fade,'TLR',0,'L','');//$fadedesc1." ".$fadedesc2
...
//			}
			...
			/*$pdf->MultiCell($width,$height,
				$pdf->Image("http://dev.3form.eu/$masterimage",$x + 4,$y,$imgwidth,'','PNG')
				,'','L','');//LR*/
			$pdf->MultiCell($width,$height,
                $pdf->Image($serverHost."/$masterimage",$x + 4,$y,$imgwidth,'','PNG')
                ,'','L','');
			...
			if($fabdesr[$loopcount]['fadeimg'] != ""){
				...
				$fadeimg = $pdf->Image($fabdesr[$loopcount]['fadeimg'],$x+4,$y+1,$imgwidth,'','PNG');//"http://dev.3form.eu/".
//				$fadedesc1 = $fabdesr[$loopcount]['fadedescr1'];//.' Width: '.$sheetwidth." imgwidth".$imgwidth
//				$fadedesc2 = $fabdesr[$loopcount]['fadedescr2'];//.$imglength
...
//			$pdf->MultiCell($width,$height,'','LR','L','');
			...
			if($fabdesr[$loopcount]['vgroove'] != ""){
				//$vgroove = $pdf->Image("http://dev.3form.eu/".$fabdesr[$loopcount]['vgroove'],$x+37,$y+$yadjust,25,'','PNG');
				$vgroove = $pdf->Image($serverHost."/".$fabdesr[$loopcount]['vgroove'],$x+37,$y+$yadjust,25,'','PNG');
                
				$yadjust = $yadjust + 20;
				$vgroovtext = $fabdesr[$loopcount]['vgroovedesc'];//$pdf->MultiCell($width,$height,,'','L','');
			}
			if($fabdesr[$loopcount]['nonvgroove'] != ""){
				//$nonvgroove = $pdf->Image("http://dev.3form.eu/".$fabdesr[$loopcount]['nonvgroove'],$x+37,$y+$yadjust,25,'','PNG');
				$nonvgroove = $pdf->Image($serverHost."/".$fabdesr[$loopcount]['nonvgroove'],$x+37,$y+$yadjust,25,'','PNG');
                
				$nonvgroovtext = $fabdesr[$loopcount]['nonvgroovedesc'];//$pdf->MultiCell($width,$height,,'','L','');
			}
			if($fabdesr[$loopcount]['sideimg'] != ""){
				$sideimg = $pdf->Image($fabdesr[$loopcount]['sideimg'],$x+4,$y+1,25,'','PNG');//"http://dev.3form.eu/".
			...
			if(!empty($fabdesr[$loopcount]['shapespath'])){
				//$shapesimg = $pdf->Image("http://dev.3form.eu/".$fabdesr[$loopcount]['shapespath'],$x+4,$y+3,58,'','JPG');
			...
			$pdf->MultiCell($width,$height,
				$shapesimg.
				$sideimg.//.$fabdesr[$loopcount]['sideimg']
				$pdf->SetY($y+21).
				$pdf->SetX(135).
				$imgdescr.
				//groove image
				$vgroove.
				$nonvgroove
				,'','L',''); //$pdf->Image("http://dev.3form.eu/$masterimage",$x,$y,20,'','PNG')
...
//					$pdf->Ln(0);
	...
	$pdf -> SetY(-80);//THis number puts the last for boxes at bottom, and must match up with $pdf->SetAutoPageBreak (false, -45);80
	...
	for ($c=0;$c<5;$c++){	
		...
		if($c==0){
			...
			$notes = $fabdesr['text']['rep_notes'].
				":<br>1. ".$fabdesr['text']['rep_notes1'].//1. DO NOT SCALE FROM DRAWINGS.
				...
				"<br>6. ".$fabdesr['text']['rep_notes6'];//6. IF IN DOUBT CALL 3FORM. ";
			
			$signaturebox = 
				"<br><b>________________________________"."_____".
				"<br>".$fabdesr['text']['rep_customername'].
				": <br><br>________________________________"."_____".
				"<br>".$fabdesr['text']['rep_date'].
				":<br><br>________________________________"."_____".
				"<br>".$fabdesr['text']['rep_signature'].":<br><br></b>";
...
//				$pdf->SetLeftMargin(10)
			...
			$summary= array($fabdesr['text']['rep_sheet'],$fabdesr['text']['rep_panels']);//this used to be three columns, but was changed to two. variables remain the same name though
			$j = $i = 0;
			$column3[$j][$i] = $fabdesr['shid'];
			$i = 1;
			$column3[$j][$i] = $panelnum;//$fabdesr['shid'].":".
			...
			$n = 1;
			$column2[$m][$n] = $fabdesr['ff']." ".$fabdesr['texture'];//"$x, $y";
			...
			$n = 1;
			$column2[$m][$n] = $fabdesr['fabrics'];//." and extra prod text"
			...
			$pdf->MultiCell($width,$height,
				...
				$pdf->SetY($y + 2).
				$pdf->productTable($summary, $column3, $column2, $fabdesr['quote_number'], $fabdesr['text']). //
				$pdf->SetLeftMargin(10)
				,'','L','');//LR on first
...
//			$pdf->MultiCell($width,$height,'','LR','L','');
			...
			$a = 1;
			$column[$a][$b] = $fabdesr['projectquote'];//." and a longer name for project"
			$a = 2;
			$column[$a][$b] = $fabdesr['text']['rep_title'];
			$a = 3; 
			$column[$a][$b] = $fabdesr['quotename'];//." and a longer name for title"
			...
			$a = 7; 
			$column[$a][$b] = "$drawingnum / $drawingnumtotal";//$x $y";
			$a = 8;
			$column[$a][$b] = $fabdesr['text']['rep_createdby'];
			$a = 9;
			$column[$a][$b] = $fabdesr['createdby'];//$createdby;
			...
			$pdf->MultiCell($width,$height,
				...
				$pdf->SetY($y + 2).
				$pdf->summaryTable($column,$fabdesr['text']). //($summary,$column3,$column2){
				$pdf->SetLeftMargin(10)
				,'','L','');//LR on first
...
//					$pdf->Ln(0);				
					...
					$loopcount++;
					$drawingnum++;
					$drawingnum = str_pad($drawingnum, 3, "0", STR_PAD_LEFT);//pad drawing number with leading zeros
				}
		...
		if(!empty($otherfab_arr)){//
			...
			foreach($otherfab_arr as $fabnum => $fabvalue){
				...
				$key = array_keys($fabvalue);//pull out the key (which is the sheet number plus other fabrication number)
				$value = array_values($fabvalue);//pull out the text
				
				//display each other fab item
				$pdf->SetFont($font,'B',$fontsize);
				$pdf->Cell($firstcell,$lineheight,"$key[0]",1,0,'L',1);
				$pdf->SetFont($font,'',$fontsize);
				$pdf->MultiCell($cw * 4, $lineheight, $value[0], 1, 'L', '');//iconv("UTF-8", "ISO-8859-1", $value[0])
				$pdf->Ln();
			}
		}
...
//srand(microtime()*1000000);
//for($i=0;$i<2;$i++){
//    $pdf->Row(array("hi", "bye and hi and hello and goodbye.  I don't know why but i can't say thigh.  WHat could be wrong.  Is this a song?"));
//}
//$pdf->Row(array("bye", "another test"));
...
//$txt = file_get_contents('/my3form_europe/optimizer/tfpdf/HelloWorld.txt');
$txt = $fabdesr['text']['rep_bf'];
$pdf->Write(8,$txt);

$pdf->Ln();

$txt = $fabdesr['text']['rep_notes3'];
$pdf->Write(8,$txt);
...
*/
		...
		if($report['fabemail'] == 1 || $fabdesr['fabemail'] == 1){
			...
			$q_username = db_query("SELECT fname, lname, mname FROM users where uid='".$r_pq['assigned_css_uid']."'");//user account
			$r_username = mysql_fetch_assoc($q_username);
			$createdby = $r_username['fname']." ".$r_username['mname']." ".$r_username['lname'];
		
			$report['fabemailtext'] = ($report['fabemailtext'] != "" ? "<b>$createdby Notes:</b><br /><i>".
				nl2br(stripslashes($report['fabemailtext']))."</i><br /><br />" : "");
			
			// email stuff (change data below)
			global $DEVSERVER;
			$to = ($DEVSERVER ? $_SESSION['username']: "fabrication@3form.eu, guinther@3form.eu, romain@3form.eu" );
			//$to = $_SESSION['username'];//fabrication@3form.eu, guinther@3form.eu, romain@3form.eu, jarom.higley@gmail.com
			//$to = "jarom.higley@gmail.com";
			$from = "ict@3form.eu";
			...
			$message = "<font face='helvetica'><p>Dear Fabrication Department,<br /><br />
				...
				https://$report[server]"."/my3form.php?mode=quote&quote=".$_SESSION[quote]."&tab=1</a><br /><br />".
				$report['fabemailtext'].
				"The fabrication report is enclosed to this e-mail for reference.".
				"<br /><br /> Regards, <br /> <br /> MyQuote Delivery System</p></font>";//<pre>".print_r($fabdesr,1)."</pre>
...
//			$search = array(" ", "+", "#", "$", "%", "^", "&", "*", "?", "/");
//			$replace = array("_", "_", "No.", "Dollar", "Percent", "", "and", "", "", "_");
//			$myquote = str_replace($search, $replace, trim($_SESSION['projectquotefulldesc']));
			$myquote = sanitize_filename($_SESSION['projectquotefulldesc']);
			 
			// attachment name
//			$filename = "Fabrication_Quote_$fabdesr[quote_id].pdf";
	...
	//***********From Test page***********
	//https://dev.3form.eu/my3form_europe/optimizer/test_pdf/test_pdf_columns.php
	// $pdf -> PrintChapter(1, 'A RUNAWAY REEF', 'Hello');
	$pdf -> PrintChapter(1, 'VARIA/CHROMA 2', '');
	// $pdf -> PrintChapter(2, 'THE PROS AND CONS', 'http://www.id.uzh.ch/cl/zinfo/fpdf/tutorial/20k_c2.txt');

	// $pdf -> PrintChapter(1, 'VARIA', '');
	// $img1=$serverDocumentRoot.'/my3form_europe/optimizer/images/195/set0sheet0.png';
	// $pdf -> Image($img1, 10, 6, 30);
	// $set1=$pdf -> Image($img1).$pdf -> Image($img1).$pdf -> Image($img1).$pdf -> Image($img1);
	// $pdf -> PrintChapter(2, 'CHROMA', $set1);
...
if ($_SESSION) {
	...
	// $resp -> addAssign("processingPDFExport", "innerHTML", $filepath);
} else {
...
$pdf_arr = array();
$pdf_arr[]= $pdfname;//add the pdf report being created
$dh = opendir($path);
...
//$merge = $pdfm
////	->addPDF('my3form_europe/optimizer/PDFMerger/samplepdfs/one.pdf', '1, 3, 4')
////	->addPDF('my3form_europe/optimizer/PDFMerger/samplepdfs/two.pdf', '1-2')
//	->addPDF($completepdf,'all')
//	->addPDF('my3form_europe/optimizer/PDFMerger/samplepdfs/three.pdf', 'all');
